...
func (h *Handler) GetDelta(w http.ResponseWriter, r *http.Request) {
	...
	d, err := h.Service.GetDelta(r.Context(), uid, chi.URLParam(r, "goalId"))
	if err != nil {
		writeAppError(w, err)
		return
	}
...
func (h *Handler) GetConsistency(w http.ResponseWriter, r *http.Request) {
	...
	c, err := h.Service.GetConsistency(r.Context(), uid, chi.URLParam(r, "goalId"))
	if err != nil {
		writeAppError(w, err)
		return
	}
	httpx.WriteJSON(w, http.StatusOK, toConsistencyDTO(c))
}
